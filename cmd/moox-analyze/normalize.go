...
func normalizeBuildingsCmd(args []string) error {
	...
	languagesDir := fs.String("languages-dir", "", "merge canonical English building names into en.json in this directory (optional)")
	if err := fs.Parse(args); err != nil {
		return err
	}
	if fs.NArg() != 1 {
		return errors.New("normalize buildings requires exactly one MOO2 installation directory")
	}
	if *out == "" {
		return errors.New("normalize buildings requires -out <path>")
	}

	bundle, err := moo2data.DecodeBuildings(fs.Arg(0))
	if err != nil {
		return err
	}
	path, err := writeJSONAtomic(*out, bundle.Rules)
	...
	fmt.Printf("normalized %d colony buildings -> %s\n", len(bundle.Rules.Buildings), path)
	return nil
}

func normalizeTechnologiesCmd(args []string) error {
	fs := flag.NewFlagSet("normalize technologies", flag.ContinueOnError)
	out := fs.String("out", "", "technologies ruleset JSON output path (required)")
	languagesDir := fs.String("languages-dir", "", "merge canonical English technology names into en.json in this directory (optional)")
	if err := fs.Parse(args); err != nil {
		return err
	}
	if fs.NArg() != 1 {
		return errors.New("normalize technologies requires exactly one MOO2 installation directory")
	}
	if *out == "" {
		return errors.New("normalize technologies requires -out <path>")
	}
	bundle, err := moo2data.DecodeTechnologies(fs.Arg(0))
	if err != nil {
		return err
	}
	path, err := writeJSONAtomic(*out, bundle.Rules)
