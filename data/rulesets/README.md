...
- Stable machine IDs are authoritative; display labels are never parsed to implement game behavior.
- Every file has a `schema_version` and `ruleset`.
- Source/provenance information remains in the normalized file so later parity work can distinguish directly observed original data from secondary-reference values.
- Exact behavior that has not yet been experimentally verified is marked as such instead of being presented as certain.
- The runtime loader validates schema and referential integrity before accepting a file.
...
- stable IDs,
- original 1.31 English labels observed in `RACESTUF.LBX`,
- standard-game Pick costs,
- simple numeric modifier values for the directly readable stat traits,
- government/ability IDs,
- known incompatibilities,
- per-field verification/provenance metadata.

The game core should eventually consume this file through the Go `internal/ruleset` loader rather than hard-coding the race designer in UI code.
