...
- within the construction column, Current and Queue are distinct: Current is shown once with authoritative PP/progress/ETA; Queue contains only future projects and is numbered from the next project onward. Queue reordering must not silently replace the current build or bypass its explicit abort/change semantics;
