...
- Slice 15.6 Gate3 Tactical mobile performance / ship readability / damage feedback IMPLEMENTED / browser-QA green for movement+rendering: per-cell reachable rectangles removed in favor of one crisp green SVG grid path over server-projected legal_moves; click-to-cell remains authority-gated. Baseline combat movement corrected from drive MaxSpeed to MinSpeed (Nuclear 10, Fusion 12) with Battle validator/regressions aligned. Tactical ship art now uses centered native nested ProceduralShipGlyph SVGs with tight Tactical viewBox instead of foreignObject/root footprint transform. Beam/impact slowed and floating layer damage numbers derive from authoritative battle_damage_applied deltas (shield cyan future-ready, armor amber, structure red). Browser 7177: 0 move-cell nodes, 1 reachable grid, 163 Tactical SVG descendants, 4 native ship vectors, Scout 10/10, 63 legal moves; legal click moved (10,9)->(11,9), movement 10->9. Plan: SLICE_15_6_GATE3_TACTICAL_MOBILE_PERFORMANCE_DAMAGE_FEEDBACK_2026-09-12.md.
