...
- Slice 15.6 Gate3 Block5C COMPLETE (`3338ead`): Galaxy star inspection and own-fleet interaction are separate sibling controls; the compact draggable picker groups fleets, renders exact persisted ship SVGs, supports whole-fleet/single-ship selection, treats fixed support ships correctly, and consumes only authoritative `fleet_move_targets` for reachable/total feedback. Canonical `0x8009` browser QA shows fleet #59 + #63, independent Scout selection, Colony Ship fallback and 0/19 reachable targets. Full Go regression and web build are green. Next: Block5D drag/drop + tap target selection, server-derived route/range feedback and staged `empire.move_fleet`.
