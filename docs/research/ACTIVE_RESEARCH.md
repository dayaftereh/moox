...
- Slice 15.6 Gate3 Block5B COMPLETE (`09a5b03`): the server now projects legal and visible-illegal fleet targets with authoritative distance/supply/range/ETA/reason data, and legal `fleet_moves` derive from that same catalog. Focused 2 pc legal / 5 pc out-of-range tests, command-validation parity, anonymous-target session coverage, full Go regression and web build are green. Next: Block5C Galaxy fleet marker + compact floating fleet/ship picker; no client-side legality formulas.
