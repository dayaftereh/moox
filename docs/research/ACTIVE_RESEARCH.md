...
- Slice 15.6 Gate3 Block5C COMPLETE + UX refinement (`3338ead`, `3503a6b`): the Galaxy fleet picker is now a compact shared system pool rather than combat/civilian tabs. All local units appear in a four-column selectable image grid with vertical overflow; normal ships use persisted procedural SVGs, strategic special ships use stable committed vector silhouettes. Own colony is signaled by a green system name. Fleet presence is now corrected to one small player-colored marker per empire/system (not per internal fleet); multiple own Combat/Civilian/Special fleets collapse to one own marker, while each visible foreign empire with fleet presence gets one separate marker in that empire's player color. Selection may span source fleets and reachable counts only intersect server-projected `fleet_move_targets`. Canonical `0x8009` browser QA shows Scout 1 + Scout 2 + Colony Ship in one 292 px picker and 0/19 targets. Full Go regression and web build are green. Next: Block5D drag/drop + tap target selection, server-derived route/range feedback and staged movement.
