...
- Slice 15.6 Gate3 Block5 planned: first close persistent per-empire visited-system/player-knowledge leakage (unvisited stars show position + color only; no true name/planets/special identity until own arrival; unknown races stay out of diplomacy until first contact), then Galaxy fleet marker direct manipulation + draggable/selectable ship picker + authoritative invalid-target range feedback + separate deterministic 3-star micro Tactical fixture; see `SLICE_15_6_GATE3_BLOCK5_GALAXY_FLEET_MOVEMENT_PLAN_2026-09-10.md`. Canonical seed `0x8009` remains unchanged.
