...
- **Click/tap the ship preview to regenerate:** the large procedural ship image is an explicit visual reroll control. Clicking/tapping it generates a new SVG/visual genome variant for the currently selected hull size while preserving design name, hull selection, installed equipment, authoritative cost/space values and all gameplay semantics. Desktop click and mobile tap must both work. A visual reroll is presentation/design identity only and must not silently change components, hull availability or construction legality.
