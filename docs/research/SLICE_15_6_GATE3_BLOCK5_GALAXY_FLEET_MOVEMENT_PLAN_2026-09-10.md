...
5D UX refinement (2026-09-11): **ACCEPTED IMPLEMENTATION** (`def7af0`)

- Supersedes the earlier explicit `Ziel wählen` / `Choose target` interaction. Opening the compact fleet picker itself is now the movement-target state: supported selected-unit profiles immediately project all destination stars as green legal or red blocked targets.
- Changing ship selection immediately recomputes the highlighted destinations from the same authoritative `fleet_move_targets`; there is no second confirmation mode and no client-side movement formula.
- The picker remains a pointer/touch overlay. Ship selection, the explicit `?` controls, info close control and picker drag gestures are handled by the picker rather than falling through to the Galaxy beneath it.
- Every ship/special-unit tile has a separate sibling `?` button (never a nested button). It opens a compact overlay popover without changing selection. Concrete Ships show existing projected hull, drive/FTL, computer, armor, shield, fuel cell/range, production cost, design/revision, weapons and the honest current strategic-damage limitation. Special strategic units show only the fields currently authoritative and retain the explicit missing-loadout/persisted-special-Ship limitation.
- `SystemDialog -> Flotten / Schiffe` now closes the SystemDialog through the existing shared picker opener and opens the exact same compact Galaxy picker. The previous large System fleet dialog has no remaining player-facing entry point; its source code is temporarily retained as dead cleanup scope rather than risking unrelated JSX deletion during this UX refinement.
- Visible canonical QA: direct Human fleet-marker open produces the 3-unit picker with no target button and `0/19` plus 19 red target rings immediately; each Scout/Colony tile has its own `?`; Scout info leaves all 3 units selected and the picker stays 292x202 while the absolute info overlay floats above it; Colony info exposes only Type/FTL/Position plus the special-unit limitation. System01 -> Flotten / Schiffe closes the SystemDialog and opens the same 3-unit picker, never the legacy large modal. Blocked-star click still reports `Unbekannter Stern · Außer Reichweite`, `8 pc · Reichweite 4 pc · ETA 4` without opening another dialog or leaking the true name.
- Validation after refinement: web build green (main JS ~276.55 kB / 71.61 kB gzip; CSS ~169.74 kB / 31.78 kB gzip), full `go test ./...` green, `git diff --check` green.
...
### Durable dual reference environment (2026-09-11)

Accepted implementation: `819880a` + `52523b2`.

The earlier plan for a single disposable 2 pc / 5 pc Block5E micro fixture is superseded by two **durable reference games** that are kept with the project and intentionally exercise complementary paths:

- `game-1` remains the canonical generated New Game (`0x8009`, 20 stars, Human seat 1 vs Darlok built-in AI seat 2). Its initial Human range is 4 pc while all 19 alternate stars are outside range, so it remains the stable blocked-target / no-hidden-name / normal-generator reference.
- `game-triangle-2pc` is the versioned `triangle-2pc-v1` reference scenario (`0x800A`): exactly three home systems for Human seat 1, Darlok built-in AI seat 2 and Psilon built-in AI seat 3. Coordinates are Human `(430,330)`, Darlok `(489,330)`, Psilon `(459,381)`. Integer map coordinates make the visual triangle near-equilateral; every edge is **exactly 2 pc under the authoritative `ceil(hypot(dx,dy)/30)` rule**.
- All three triangle home systems deliberately have symmetric starting conditions: the normal ruleset homeworld plus two additional planets, standard Average technologies, the same starting population/economy, two Scouts and one Colony Ship. Each empire initially knows/visits only its own home. Foreign system names and empire identities remain hidden until normal visit/contact rules reveal them.
- At initial Standard Fuel Cells / Nuclear Drive, the Human triangle picker therefore has exactly `Ziele 2/2`; both anonymous destinations are server-projected legal at `2 pc · Reichweite 4 pc · ETA 1`.
- Visible browser QA on 7171 staged the first anonymous destination with all three Human units selected and received `2 Bewegungsorder geplant` (combat Scout fleet + Colony Ship fleet) with the same 2 pc/range4/ETA1 authority values. The turn was deliberately **not submitted**, leaving the reference game clean on turn 1.
- Dev server flag `-reference-games` explicitly pre-registers both worlds. Normal/production-neutral startup remains empty when the flag is absent. The canonical development 7171 service now runs with `-reference-games`, so both references are recreated deterministically on dev-server restart even while the separate live-save persistence seam remains open.
- Future Slice/Gate work should preserve and update both references: `game-1` for realistic generated/blocked behavior and `game-triangle-2pc` for immediate movement, first-contact, multi-empire fleet-marker and Tactical integration acceptance.
### Fleet route lifecycle (2026-09-11)

Accepted interaction contract:

- While a move is only a local draft **before `Fertig`**, the route is editable. A legal destination click creates/replaces the selected Fleet move drafts and renders one grouped green dashed route. Clicking another legal destination retargets those drafts. Clicking the source/home system removes those selected drafts and the planned route disappears.
- A blocked destination never stages or replaces an order. It renders a red dashed diagnostic route from source to the blocked star and keeps the authoritative reason/distance/range/ETA feedback; hidden destination identity stays hidden.
- After `Fertig` has committed the move and the Fleet enters strategic transit, the flight is **immutable until arrival**. There is no cancel or retarget action on an in-transit Fleet. Both fixed special fleets and ordinary combat fleets are regression-tested to reject a second move command while `at_system_id == 0` without mutating the transit state.
- Authoritative transit state now persists `source_system_id`, `destination_system_id`, `remaining_turns` and `transit_turns_total`. Save/load tests cover the route metadata; arrival clears all route fields.
- Player projection adds server-derived `route_distance_parsecs` and `remaining_distance_parsecs`; React never computes strategic distance. Remaining parsecs use the same authoritative strategic distance and discrete turn progress. Example test: a 3 pc / ETA2 flight projects 3 pc remaining at start and 2 pc after the first transit advance.
- The Galaxy renders committed transit as a green dashed line from the Fleet's current visual position toward its destination. The travelling Fleet marker remains clickable only for status. Its popover shows unit count, remaining parsecs and ETA plus the explicit notice that the route cannot be changed until arrival. No movement-planning controls are exposed from the transit marker.
- Because `game-triangle-2pc` has ETA1 on every edge, a submitted move arrives during the same turn resolution and therefore has no intermediate transit-marker frame. Multi-turn transit UI data/lifecycle is backed by the authoritative projection and movement tests; `game-1` remains the canonical blocked-route visual acceptance world.
- Visible QA: `game-1` blocked click produced exactly one red dashed route (`rgb(255,120,135)`, dash 7/5), zero planned routes, and `Unbekannter Stern · Außer Reichweite · 8 pc · Reichweite 4 pc · ETA 4` with no System dialog/name leak. Triangle pre-submit QA retained one green grouped route while two source-Fleet drafts were staged, retargeting stayed at two drafts, source click cancelled both, and closing the picker left the planned route visible.
### Fleet-marker route anchor and transit inspection refinement (2026-09-11)

- Planned green and blocked red routes now originate visually at the center of the own Empire Fleet marker, not at the source star/system center. The route still uses the authoritative source/destination IDs; only the presentation anchor moves to the already-rendered Fleet marker slot.
- Visible geometry QA measured the red route start within 0.02 px of the Fleet-marker center while the source-star center was about 26.9 px away. The green planned route was within 0.33 px of the Fleet-marker center and about 26.7 px from the source-star center. Pan/zoom keeps line and marker in the same transformed Galaxy layer.
- Clicking an in-transit Fleet marker now opens the same compact Fleet visual language as the stationary picker, but in a strict read-only transit mode. It lists the travelling units with their normal ship/special-vessel glyphs and keeps each unit's `?` information control available.
