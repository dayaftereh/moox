...
- Slice 04 Combat Fleet movement / merge / split is closed with implementation commit `f297480`; no gameplay slice is currently open.
- Three remaining `PLANNED_05...07` specifications prepare the next roadmap. Slice 05 Command Points / ship maintenance is the recommended next objective and must begin with a fresh Gate 1 plus exactly one dated `_OPEN_` marker.
