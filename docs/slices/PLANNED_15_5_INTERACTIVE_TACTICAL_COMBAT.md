# Planned slice 15.5 - Interactive 2D tactical combat
...
- [ ] Re-audit Slice-07 evidence and current BattleSession/Session/App/browser battle surfaces.
- [ ] Research original MOO2 tactical coordinate/grid, movement, speed/cost, initiative, range and any facing/turning coupling required by the minimum interactive baseline.
- [ ] Distinguish proven original behavior from deliberate MOOX UI modernization.
- [ ] Define the minimum supported combatant/multi-ship breadth for the first credible interactive battle.
- [ ] Identify required Core/Battle/Session player-safe reads, legal-action projections and command envelopes for movement and targeting.
- [ ] Define desktop mouse and mobile touch battlefield interaction, including pan/zoom and no-hover requirements.
- [ ] Define deterministic/browser regression strategy from strategic Encounter entry through battle result return.
- [ ] Present the Gate-2 authoritative Tactical contract before implementation.
