...
**TACTICAL MOBILE PERFORMANCE / SHIP READABILITY / DAMAGE FEEDBACK IMPLEMENTED / BROWSER-QA GREEN.** Legal movement no longer materializes hundreds of rounded SVG cells; the server-projected legal set is rendered as one crisp green grid path and battlefield clicks are accepted only when the rounded integer cell exists in that authoritative set. Baseline movement now uses ruleset MinSpeed rather than MaxSpeed (Nuclear 10, Fusion 12), with the Slice15.5 Battle validator and regressions updated. Tactical ship art is native centered nested SVG with a tight viewBox; persisted visual_genome remains authoritative when present and untouched starting Scouts truthfully use deterministic fallback. Beam impact timing is slower and floating damage numbers are derived from authoritative battle_damage_applied layer deltas: armor amber, structure red, shield cyan reserved for future shield authority. Isolated Triangle browser QA on 7177 proved one reachable-grid path, zero move-cell nodes, four ship SVGs/no foreignObject, Scout 10/10, and an authoritative legal-grid move from (10,9) to (11,9). See SLICE_15_6_GATE3_TACTICAL_MOBILE_PERFORMANCE_DAMAGE_FEEDBACK_2026-09-12.md.
