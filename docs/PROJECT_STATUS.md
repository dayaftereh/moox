...
| `economy.json` | base research/income/industry + Population/Sustenance + Food/Freighter/Starvation + Population Growth modifiers + Aquatic + gravity + starting-government + local Morale rules | base/contextual Economy, original-style round-robin insufficient-Freighter import priority, authoritative system-blockade exclusion from Food import/export/sale pools, Freighter balancing/cost snapshot, surplus Food valuation, Starvation, 50-PP Freighter Fleet production (+5 Freighters), authoritative modeled Treasury settlement, Population relocation through the shared Freighter pool, Housing, Microbiotics, Universal Antidote and Cloning Center growth effects implemented; Fleet-derived blockade production, Population-capacity buildings/transitions, mixed Population cohorts, missing original Treasury categories, broader buildings/pollution and advanced-government effects remain |
...
- Biospheres / Advanced City Planning / terraforming Population-capacity transitions, Fleet/Diplomacy-derived blockade production, mixed Population cohorts for the later original Food-priority passes, and missing original Treasury categories/deficit policy,
