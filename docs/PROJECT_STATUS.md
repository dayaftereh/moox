...
The strategic Fleet/Diplomacy blockade-production checkpoint is complete. Core schema 15 now persists minimal strategic Fleet location/role and directed Empire relations, derives system blockades from Colony presence plus stationary hostile combat Fleets, and the full Go test/vet baseline is green.
...
- missing original Treasury categories/deficit policy,
...
Canonical strategic Fleet/Diplomacy blockade production is complete and preserved in `docs/research/FLEET_DIPLOMACY_BLOCKADES_2026-08-29.md`. The next queued slice is the missing original Treasury income/Maintenance categories and deficit/scrap policy, limited to categories whose dependencies are already authoritative.
