...
Slice 14 **Live GameSession save/resume baseline** is currently open. Gate 2 is frozen by user approval and Gate 3 implementation is complete in `a2c4ccd`: LiveSnapshot v1 preserves Core23 plus Session/Battle continuation state, simulation compatibility, compacted ruleset fingerprinting, stable interactive save boundaries, production-resolver/controller scope and atomic Host/HTTP import/restore. Exact canonical AI save-at-Turn-250 continuation is proven; Gate 4 independent QA/closure is next. Slices 15-17 remain prepared and unopened. `docs/research/ACTIVE_RESEARCH.md` remains the authoritative handoff.
