...
Technology ownership is now represented by both `Empire.KnownTechnologyFieldIDs` and sorted `Empire.KnownTechnologyIDs`. Active research is persistent `ResearchState` with an original tech-field ID, server-materialized technology IDs and RP-native `float64` progress (`progress_rp`).

Normal strategic resolution now calculates and rolls research breakthroughs automatically after Colony economy/construction resolution. MOOX preserves fractional `float64` RP through accumulation and applies rounding only at the discrete breakthrough-percent boundary; the verified MOO2 1.31 curve, 1..100 roll semantics, active-project RNG consumption and no-overflow completion behavior remain the rule reference. `empire.research_progressed` records the chance/roll plus numeric and speaking Technology IDs; `empire.research_completed` performs the ownership transition. `GameSession.CompleteResearchField` remains a lower-level explicit commit seam for tests/admin-controlled transitions, not the normal Human/AI gameplay path. No frontend, network client or AI adapter should mutate technology ownership directly.

Automatic breakthrough/overflow behavior now covers both standard fields and Hyper-Advanced repeat fields. Hyper fields 75..82 use server-owned `repeat_field` legal actions, sparse per-Empire completed-level state, and dynamic strategic cost `15000 + completed_levels*10000 RP`; they carry no concrete Technology IDs and remain active after breakthrough with RP reset to zero.

Research selection is authoritative and race-aware: `GameSession.ResearchChoices` projects `all`, `choose_one`, `fixed_one`, or Hyper-Advanced `repeat_field` per TechField even while a project is active. `empire.select_research` starts a project or switches to another legal field/application; switching transfers `ProgressRP` exactly and emits `empire.research_switched`. `technology_id` is accepted only for `choose_one`; General/Creative, Uncreative-fixed, and Hyper-repeat ownership remains server-derived. Human, AI, Wails and remote transports consume this same legal-action surface.
...
After local Economy materialization, empire Food logistics applies available Freighter capacity and emits `empire.food_logistics_resolved` when relevant. The original MOO2 1.31 apply order is then mirrored: Research consumes the already-materialized pre-growth RP snapshot, Population Growth/Starvation is applied, and Construction consumes the already-materialized pre-growth `PopulationDynamics.ProductionAvailable`. Only afterwards is the next-state Economy/logistics preview recalculated. Newly changed Population therefore cannot retroactively change that turn's consumed RP or PP.
