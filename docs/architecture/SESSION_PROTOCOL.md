...
Research selection is authoritative and race-aware: `GameSession.ResearchChoices` projects `all`, `choose_one`, or `fixed_one` per TechField even while a project is active. `empire.select_research` starts a project or switches to another legal field/application; switching transfers `ProgressRP` exactly and emits `empire.research_switched`. `technology_id` is accepted only for `choose_one`; General/Creative and Uncreative-fixed ownership remains server-derived. Human, AI, Wails and remote transports consume this same legal-action surface.
