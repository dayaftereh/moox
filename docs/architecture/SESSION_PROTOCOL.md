...
After local Economy materialization, empire Food logistics automatically applies available Freighter capacity and emits `empire.food_logistics_resolved` when relevant. Construction consumes `PopulationDynamics.ProductionAvailable`; Research then consumes current-turn RP. The current MOOX order resolves positive Growth or Starvation afterwards and recalculates the next-state Economy/logistics preview. A conflicting secondary description of the original 1.31 turn order is tracked explicitly rather than silently changing this contract.
