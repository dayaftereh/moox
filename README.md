...
Normalized rules live under `data/rulesets/`. The first committed dataset is `data/rulesets/moo2-1.31/race_traits.json`, generated by the pure-Go analyzer and validated by the Go runtime loader. Game code should consume stable IDs/numeric values from these files rather than deriving rules from UI labels.
