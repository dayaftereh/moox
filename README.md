...
**Slice state:** Slices **01-14 and 15.1 are closed**. Slice 15 is the browser-playability parent epic. **Slice 15.2 is open; Gates 1-3 are complete and Gate 4 independent QA/closure is pending.** Slices 15.3-15.6 and 16-17 remain prepared; **Slice 20 Espionage/Intelligence is now reserved/prepared** by product decision.
...
| 15.2 | **open; Gates 1-3 complete, Gate 4 pending** | Functional strategic gameplay HMI |
...
| 16 | prepared | New Game / preset-race breadth |
| 17 | prepared | Military ship-design component / weapon breadth |
| 20 | prepared | Espionage / intelligence baseline |
...
- `7c49e90` - Core schema 14 race-aware Population cohorts, exact four-pass Food priority, heterogeneous capacity and cohort-aware transfers.
...
The active runtime follows a domain-native numeric architecture: continuous quantities use `float64` (`PopulationState.Cohorts` plus derived Population aggregate helpers, Food, PP, RP, BC and Construction progress), while genuinely discrete IDs/counts remain discrete. Population Growth now uses the classic curve directly in Population units, and freshly grown Population cannot retroactively contribute PP/RP to the turn that produced it. Rounding occurs only at explicit gameplay-rule boundaries. The Economy checkpoint includes Food/Freighter balancing and starvation. Research now models General `all`, ordinary `choose_one`, Creative `all`, and Uncreative server-fixed `fixed_one` application semantics. Active research can now be switched while preserving the complete accumulated RP pool. Direct original-executable analysis has resolved the 1.31 turn order: current RP/PP are materialized pre-growth; Research is applied first, then Population Growth/Starvation, then Construction using the pre-growth PP snapshot. Direct original-executable analysis has now also resolved Uncreative initial-selection timing/RNG ownership: fixed applications are generated during new-game player initialization from the shared New Game RNG. Hyper-Advanced repeated-field progression and Advanced-start randomized/race-aware technology ownership are now implemented from direct original-executable evidence. Advanced start uses the shared New Game RNG, cross-Empire competition weighting, and exactly 19 weighted extra grants after the Average baseline. Authoritative external Technology grants are now implemented as a server-owned transition: they add the concrete Technology without inventing TechField completion, emit Observer/replay metadata, and apply the verified Uncreative repair through the authoritative State RNG. Freighter Fleet production is implemented through the shared Construction legal-action model: 50 PP creates 5 Empire Freighters, with authoritative queue/progress/completion events. Treasury settlement is now authoritative as well: New Game starts at the directly verified 50 BC, and each strategic turn settles currently modeled Tax + surplus-Food income against Building + active-Freighter Maintenance before Research. Direct original-executable analysis now also resolves insufficient-Freighter imports and blockade eligibility: constrained Food is allocated round-robin across deficit Colonies, while blockaded Colonies are excluded from both Food import and export/sale pools through authoritative StarSystem state. Population relocation through the shared Freighter pool is now authoritative: interstellar Settlers reserve five Freighters before Food, same-system moves are immediate, original ETA/Warp-Drive/Trans-Dimensional rules are modeled, and resolved transfers release capacity before the final Food snapshot. Core state schema is now 16. Housing, medicine and Cloning growth modifiers remain authoritative; Population capacity now layers Advanced City Planning +5 and Biospheres +2 after the original race/planet rules, while Terraforming and Gaia Transformation are semantic one-shot planetary projects that mutate physical climate without persisting as Buildings. Race-aware Population cohorts are now authoritative for mixed/conquered Colonies, including source-race Economy, exact four-pass Food priority, per-origin Growth/Starvation and heterogeneous capacity. Strategic system blockades are now derived deterministically from Colony presence, stationary combat Fleets and directed hostile relations before Population-transfer arrivals and the final Food snapshot. The Treasury Maintenance ledger is now directly resolved as Building/Colony, active Freighter, Ship command-point overage, Spy, outgoing Tribute and Officer/Leader categories. The dependency-safe runtime corrections are implemented: Population-transfer reservations and Food use share the aggregate active-Freighter Maintenance bucket with whole-BC truncation, and surplus-Food BC income uses the original whole-BC conversion boundary. Complete original deficit liquidation remains deferred until its Ship/Spy/Leader/treaty dependencies exist. The first Colony Ship build -> move -> arrive -> colonize vertical loop is now authoritative through Core/Game/Session, including Observer/save-load coverage and a second Colony. No slice is open. Slice 08 **Authoritative server, web HMI and transport baseline** is the next prepared objective and must begin with a fresh Gate 1.
## Research documents
...
- `docs/research/COLONY_SHIP_MOVEMENT_COLONIZATION_2026-08-30.md` - direct Colony Ship production, Fuel range, strategic transit and colonization evidence plus schema-16 implementation checkpoint.
...
- `docs/research/BLOCKADE_FOOD_LOGISTICS_2026-08-28.md` - direct blockade predicate/timing evidence plus authoritative import/export/surplus exclusion.
