...
func syntheticAssetBuildings() *ruleset.BuildingsFile {
	...
	for order, spec := range buildingSpecs() {
		file.Buildings = append(file.Buildings, ruleset.Building{
			...
			ProductionCostPP:           60,
			ProductionCostVerification: "test",
			ProductionCostSource:       ruleset.FieldProvenance{SourceID: "test-building-table"},
			MaintenanceBC:              1,
			MaintenanceVerification:    "test",
			MaintenanceSource:          ruleset.FieldProvenance{SourceID: "test-building-table"},
