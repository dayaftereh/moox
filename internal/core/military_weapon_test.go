...
func TestShipWeaponMountValidationAndSchema21RoundTrip(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if loaded.SchemaVersion != 21 || !reflect.DeepEqual(loaded.ShipDesigns[0].Spec.Weapons, spec.Weapons) {
