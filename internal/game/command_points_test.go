...
func TestCommandPointSnapshotRoundTripsSchema23(t *testing.T) {
	_, resolver := commandPointTestResolver(t)
	...
	if loaded.SchemaVersion != 23 || loaded.Empires[0].CommandPoints != state.Empires[0].CommandPoints || loaded.Empires[0].Treasury != state.Empires[0].Treasury {
